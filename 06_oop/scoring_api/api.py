...
class BaseField:
    ...
    def __set__(self, instance, value) -> None:
        if not value and self.nullable is False:
            logging.exception(
                f"Value error: Non-nullable {self.name} field cannot be None"
            )
    ...
    def __init__(self, **kwargs) -> None:
        ...
        for field_name, descr in user_class_attrs.items():
            if field_name not in kwargs.keys():
                if descr.required:
                    logging.exception(
                        f"Value error: Required field {field_name} must be specified"
                    )
                ...
                if not descr.nullable and kw_value is None:
                    logging.exception(
                        f"Value error: Non-nullable {field_name} field cannot be empty"
                    )
...
class BirthDayField(DateField):
    def __set__(self, instance, value) -> None:
        ...
        if datetime.datetime.now().year - date.year > 70:
            logging.exception(
                f"Value error: Field {self.name}, value {value} is bigger than 70"
            )
